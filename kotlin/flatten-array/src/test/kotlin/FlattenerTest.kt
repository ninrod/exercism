...
class FlattenerTest {
    ...
    fun flatListIsUnchanged() {
        ...
        Flattener.flatten(flatList) `should equal`  listOf(0, 1, 2)
    }

    @Test
    fun flattens2LevelNestedList() {
        val nestedList = listOf(1, listOf(2, 3, 4, 5, 6, 7), 8)
         Flattener.flatten(nestedList) `should equal` listOf(1, 2, 3, 4, 5, 6, 7, 8)
    }

    @Test
    fun flattens5LevelNestedList() {
        val nestedList = listOf(0, 2, listOf(listOf(2, 3), 8, 100, 4, listOf(listOf(listOf(50))), -2))
        Flattener.flatten(nestedList)  `should equal`  listOf(0, 2, 2, 3, 8, 100, 4, 50, -2)
    }
    ...
    fun flattens6LevelNestedListWithNulls() {
        ...
        Flattener.flatten(nestedList) `should equal` listOf(0, 2, 2, 3, 8, 100, -2)
    }
    ...
    fun allNullNestedListReturnsEmptyList() {
        ...
        Flattener.flatten(nestedList) `should equal` emptyList<Any>()
    }
}
